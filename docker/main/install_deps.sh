...
if [[ "${TARGETARCH}" == "amd64" ]]; then
  ...
  sed -i -E "/^Components: main$/s/main/main contrib non-free non-free-firmware/" "/etc/apt/sources.list.d/debian.sources" \
    ...
    gpg --no-default-keyring --keyring=./temp.gpg --keyserver hkps://keyserver.ubuntu.com --recv-keys 0C0E6AF955CE463C03FC51574D098D70AFBE5E1F | gpg --yes --dearmor --output /usr/share/keyrings/intel-graphics.gpg ./temp.gpg
